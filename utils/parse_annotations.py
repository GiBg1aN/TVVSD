...
def main():
    ...
    tuhoi_df = pd.read_csv('crowdflower_result.csv', encoding='ISO-8859-1')
    tuhoi_df = tuhoi_df[['image_name', 'tag1', 'orig_tag1']]
    tuhoi_df["tag1"] = tuhoi_df["tag1"].apply(lambda x: x.split('\n')[0])
    # tag1: verb
    # orig_tag1: object

    train_df = pd.DataFrame(caption_train.get("annotations"))
    val_df = pd.DataFrame(caption_val.get("annotations"))
    ...
    new_df3 = pd.DataFrame(columns=['caption', 'image_id'])
    ...
    for img_name in img_list:
        if img_name.startswith("COCO_train2014_"):
            ...
            row = tuhoi_df[tuhoi_df['image_name'] == img_name]
            caption = 'person ' + row.tag1 + ' ' + row.orig_tag1
            new_df3 = pd.concat([new_df3, pd.DataFrame({'caption': caption, 'image_id': img_name})])

    new_df = pd.concat([new_df1, new_df2])
    new_df = new_df.drop(new_df.columns[1], axis=1)

    if (len(new_df1.image_id.unique()) +
            len(new_df2.image_id.unique()) == len(new_df.image_id.unique())):
